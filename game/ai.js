...
export const ai = {
  ...
  control: {human: null, raptor: null},
...
ai.humanSpaceDistance = sequence(
  ...
  (_) => sequence(bd.nHumanSpaces).map((_) => null)
);
...
ai.raptorSpaceDistance = {};
{
  ...
  ai.raptorSpaceDistance.move = new Array(
    ...
    (_) => new Array(bd.nRaptorSpaces).fill(null)
  );
...
ai.raptorHumanDistance = (gameState) => {
  ...
  const dm =
    raptors.map((_) => humans.map((_) => null));
...
for (const species of ['human', 'raptor', 'trex']) {
  const dieM = dice[species].movement;
  if (species === 'trex') {
    ai.prob.dice.trex.move =
      arraySum(dieM) / dieM.length;
    continue;
  }
...
for (const species of ['human', 'trex']) {
  ai.trexDanger[species] = new Array(
    ...
    (_) => new Array(bd.trexStart + 1).fill()
  );
